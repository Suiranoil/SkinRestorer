...
public final class SkinService {
    ...
    public static Collection<ServerPlayer> applySkin(
            MinecraftServer server, Iterable<GameProfile> targets, SkinValue value, boolean save) {
        ...
        for (var profile : targets) {
            ...
            if (PlayerUtils.areSkinPropertiesEquals(playerValue.value(), skin)) continue;
    ...
    public static Collection<ServerPlayer> applySkin(
            MinecraftServer server, Iterable<GameProfile> targets, SkinValue value) {
    ...
    public static CompletableFuture<Result<Collection<ServerPlayer>, String>> setSkinAsync(
            MinecraftServer server, Collection<GameProfile> targets, SkinProviderContext context, boolean save) {
        return CompletableFuture.supplyAsync(
                ...
                .thenApplyAsync(
                        result -> {
                            if (result.isEmpty())
                                return Result.<Collection<ServerPlayer>, String>error(
