...
public final class ProvidersConfig implements GsonPostProcessable {
    ...
    private List<CustomProviderConfig> getValidatedCustom() {
        if (this.validatedCustom == null) this.rebuildValidatedCustomProviders();

        if (this.validatedCustom == null) this.validatedCustom = List.of();
        ...
        for (var config : this.custom) {
            ...
            if (!seenNames.add(config.name())) {
                SkinRestorer.LOGGER.warn(
                        "Duplicate enabled custom provider name '{}' found; keeping the first one", config.name());
                continue;
            }
