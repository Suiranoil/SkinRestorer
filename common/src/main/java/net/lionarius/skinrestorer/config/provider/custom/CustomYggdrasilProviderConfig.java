...
public final class CustomYggdrasilProviderConfig extends CustomProviderConfig {
    ...
    public Result<SkinProvider, String> createSkinProvider(SkinSigner skinSigner) {
        if (!this.hasConfiguredUrls()) return Result.error("servicesUrl and sessionUrl are not fully configured");
    ...
    public void gsonPostProcess() {
        ...
        if (!this.baseUrl.isEmpty()) {
            if (this.servicesUrl.isEmpty()) this.servicesUrl = this.baseUrl;

            if (this.sessionUrl.isEmpty()) this.sessionUrl = this.baseUrl;
        }
    }
}
