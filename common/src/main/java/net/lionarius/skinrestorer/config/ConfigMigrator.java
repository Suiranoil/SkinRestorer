...
public final class ConfigMigrator {
    ...
    public static JsonObject migrateToLatest(JsonObject json) {
        ...
        if (version > CURRENT_VERSION)
            throw new IllegalStateException("Config version %d is newer than supported version %d".formatted(version, CURRENT_VERSION));

        if (version < CURRENT_VERSION)
            SkinRestorer.LOGGER.info("Migrating config from version {} to {}", version, CURRENT_VERSION);
